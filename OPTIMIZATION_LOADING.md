> **Spec contract (source of truth):** [docs/specs/02-loading.md](docs/specs/02-loading.md) · [docs/specs/03-performance.md](docs/specs/03-performance.md). This file is historical notes — update the specs in the same PR when changing loading/perf.

> **2026-09-06:** **Ultra / Simples presets** — removed all live FPS adaptation on the stream path. No `holdForTargetFps` / Valve HOLD, no LoadGovernor level climb, no MemoryGuardian FPS radius shrink/expand, no QualityAdapter live ladder. Fixed knobs per preset (default **Ultra**). Hitch HUD stays observational only. Park-freeze residency floor dropped (fixed R). Keep orchard + biomes √10 + vista fence + phys pin + Fila do foco + progressive boot.

> **2026-09-06:** Park-freeze load radius early — when the focus cell is unchanged for ~1.5s (armed), freeze `effectiveLoadRadius` at the **max Guardian R for that cell** (do not wait for `remaining===0`). MemoryGuardian `residencyFloor` keeps world/building growers inside the frozen disk (no dispose→reload thrash when R oscillates 70↔160). `continueAfter` never expands past the freeze; soft-cap may still drain in-frozen-disk street glTFs so Fila do foco can hit 0. End streets/nature when core remaining for frozen R is 0. Carpet optional (#100), vista (#98).
