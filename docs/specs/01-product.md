...
- **Day/night (fases 1–2):** `DayNightController` owns Sky + sun orbit + hemi/sun/moon + stars; HUD **Hora** scrub 0–24h; hora persiste em `localStorage` `city-day-night-v1` `{ hours }` (boot restore; scrub / play / pagehide). Sem PMREM/frame, sem `@takram/three-atmosphere`, sem FPS HOLD. Postes GLB acendem à noite (`nightFactor` + pool ≤6 SpotLights + Point fill, wash asfalto/calçada/grama do meio-fio). HUD estudo **Iluminação** (minimizable, localStorage) tweaka `setParams` ao vivo. Bússola HUD (N/S/L/O) após Começar.
...
- Botão `#car-visual-btn`: ciclo **Porsche → Mercedes (otimizada) → Mercedes (high-poly) → Defender → quadrado**.
- Labels PT: `Carro: Porsche` / `Carro: Mercedes (otimizada)` / `Carro: Mercedes (high-poly)` / `Carro: Defender` / `Carro: quadrado`.
- A/B: botão **Comparar A/B** offset +3 m no X local quando Mercedes/orig carregados — contrato em `06-vehicles.md`.
- Load times: painel **Load carros** (`getLoadStats()`) lista otimizada + high-poly pré-pipeline para carros do pipeline.
...
- Free-flight RMB pan: horizontal = XZ strafe; vertical = world Y elevate (drag down → camera up). No look rotate.
