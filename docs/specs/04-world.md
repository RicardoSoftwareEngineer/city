...
Street lamps → Quaternius/CC0 `public/models/street/lamp_post.glb` instanced via `StreetFurniture` / `collectStreetlightPoses` (no procedural cylinders). Night-only: shared bulb emissive × `nightFactor` + capped PointLight pool (`StreetLightsController`, nearest N to camera; lights `castShadow` off). **MUST NOT** one PointLight per pole.
