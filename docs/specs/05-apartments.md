...
- **MUST** shared room bake include loft furniture shortlist (`/models/apartments/loft_furniture.glb`: sofa, plant, coffee+legs, console, lamp, chair) as **MeshBasic albedo-only** (textures ≤256) merged into the same InstancedMesh material buckets — never unique furniture per unit; rescale to the 3.2×3.6×2.75 template so silhouettes read through the pane. GLB pieces MUST be floored + Y-up (extract Rx(-90) for Z-up FBX meshes); yaw-only coherent living layout. Do **not** import loft architecture (brick/beams/windows/cityscape) or loft Point light.
