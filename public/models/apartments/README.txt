...
Sandbox loads this file with ?v=apt-example-4 (hard-refresh if the browser caches).
Street catalog picker: empty shell on asphalt + labeled samples on the road; drag (or click) / __cityAptExample.addFromCatalog(name) to stage into the room; drag in-room pieces to reposition. Sofa: keep Y-up + DoubleSide MeshBasic (not flat/black).
