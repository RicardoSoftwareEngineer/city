...
import { getActivePreset } from '../../engine/qualityPresets.js';
...
function vegTuning() {
  ...
  return {
    spacing: Math.max(1, p.vegSpacingScale || 1),
    poses: Math.max(0.15, p.vegPoseCapScale || 1)
  };
}

function scaleSpacing(n) {
  return n * vegTuning().spacing;
}

function scalePoses(n) {
  return Math.max(1, Math.round(n * vegTuning().poses));
}
...
export async function registerNearVegetation(stream, _parentGroup, _ox, _oz) {
  const tallGrass = await scatterGridAsync({
    spacing: scaleSpacing(3.5),
    seedSalt: 11,
    scaleMin: 0.7,
    scaleMax: 0.95,
    halfExtent: NEAR_HALF,
    maxPoses: scalePoses(NEAR_POSE_CAP),
  ...
  const bushesNear = await scatterGridAsync({
    spacing: scaleSpacing(22),
    seedSalt: 55,
    scaleMin: 0.9,
    scaleMax: 1.35,
    halfExtent: NEAR_HALF,
    maxPoses: scalePoses(400),
...
export async function registerFarVegetation(stream, parentGroup, ox, oz) {
  ...
  const tallFar = await scatterGridAsync({
    spacing: scaleSpacing(7),
    seedSalt: 111,
    scaleMin: 0.7,
    scaleMax: 0.95,
    halfExtent: FIELD_HALF,
    minHalfExtent: NEAR_HALF,
    maxPoses: scalePoses(FAR_POSE_CAP),
    accept: (x, z) => acceptFieldGrass(x, z, { maxSlope: 0.55 })
  });
  ...
  const bushes = await scatterGridAsync({
    spacing: scaleSpacing(24),
    seedSalt: 155,
    scaleMin: 0.9,
    scaleMax: 1.35,
    halfExtent: FIELD_HALF,
    minHalfExtent: NEAR_HALF,
    maxPoses: scalePoses(800),
    accept: (x, z) => acceptBush(x, z)
  });
  ...
  const plants = await scatterGridAsync({
    spacing: scaleSpacing(32),
    seedSalt: 66,
    scaleMin: 0.85,
    scaleMax: 1.2,
    halfExtent: FIELD_HALF - 20,
    maxPoses: scalePoses(600),
  ...
  const rocks = await scatterGridAsync({
    spacing: scaleSpacing(28),
    seedSalt: 77,
    scaleMin: 0.7,
    scaleMax: 1.4,
    halfExtent: FIELD_HALF,
    maxPoses: scalePoses(500),
  ...
  const horizon = scatterHorizonPines({ count: scalePoses(36), minR: HORIZON_MIN, maxR: HORIZON_MAX });
...
export async function registerDenseCarpet(stream) {
  const wideTallNear = await scatterGridAsync({
    spacing: scaleSpacing(2.4),
    seedSalt: 12,
    scaleMin: 0.9,
    scaleMax: 1.25,
    halfExtent: DENSE_HALF,
    maxPoses: scalePoses(DENSE_POSE_CAP),
